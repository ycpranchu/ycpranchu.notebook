...
Exceptions (`Synchronous`, Software Interrupts) - Exceptions are `produced by the processor` while executing instructions either in response to a `programming error` (e.g., divide by zero) or `abnormal condition` (e.g., call a system call or a page fault).
...
<Callout type="info" emoji="ℹ">
Interrupt Request (IRQ) - `Specific interrupt` is associated with a `specific device`.
</Callout>
