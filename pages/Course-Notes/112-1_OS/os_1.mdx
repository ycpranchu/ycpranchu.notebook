...
Each process shall have their own **virtual address space (1GB | 3GB)**. if physical memory -> hard to programming
...
![](https://hackmd.io/_uploads/r1cNSaBg6.png)

- Just switch pointer (User mode to Kernel mode)
...
* Software-generated interrupts (Traps or Exceptions)
    - Specific requests from user processes (**System Call**, such as Read/Write) -> program library
    - Some errors (e.g. invalid memory access or page faults)

![](https://hackmd.io/_uploads/ryNq8TrgT.png)

Context Switch: time inturrupt
...
* Storage response time catches up with **context switch overhead** (1~10 us). -> CPU don't need to wait
* The **overhead** of running **file system** becomes critical (e.g., page cache).
* Huge memory demands increases deployment cost. (Right page size?) -> problem: **page table**
